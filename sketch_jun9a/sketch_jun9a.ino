...
// BME280 (FIXED: Removed &Wire because your library version doesn't support it)
    bme_ok = bme.begin(0x76); 
    if (!bme_ok) {
        // Try alternate standard address just in case
...
void loop() {
    ...
    // ── BME280 / BMP280 ───────────────────────────────────────────────────────
    float temp     = bme_ok ? bme.readTemperature()            : -999.0f;
    float humidity = bme_ok ? bme.readHumidity()               : 0.0f;   // BMP280 ไม่มี humidity
    float alt_baro = bme_ok ? bme.readAltitude(SEA_LEVEL_HPA)  : 0.0f;
    Serial.println("Temp :");
    Serial.println(temp);
